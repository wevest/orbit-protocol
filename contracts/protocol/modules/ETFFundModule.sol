...
import { ICustomOracleNavIssuanceModule } from "../../interfaces/ICustomOracleNavIssuanceModule.sol";
 ...
 */
contract ETFFundModule is ModuleBase, ReentrancyGuard {
    ...
    constructor(IController _controller, IERC20 _usdcToken) public ModuleBase(_controller) {
     ...
     * User buy setToken
     *
     * @param _setToken                 Selected setToken
     * @param _amount                   Amount for buying
     * @return uint256                  Amount of sendTokens used in the trade
     */
    function buy(
        ISetToken _setToken,
    ...
    {
        uint amount = _amount;
        ISetToken setToken = _setToken;
        require(amount > 0, "Amount should be greater than 0");
        require(USDC_TOKEN.allowance(msg.sender, address(this)) >= amount, "Allowance amount is insufficient");
        require(USDC_TOKEN.balanceOf(msg.sender) >= amount, "Insufficient USDC token in the address");
        USDC_TOKEN.transferFrom(msg.sender, address(this), amount);

        // ETF module gets information about the selected ETF (components, weights)
        address[] memory components = setToken.getComponents();
        uint[] memory weights = setToken.getWeights();
        uint[] memory quantities = new uint[](weights.length);

        // Get Modules of setToken
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            quantities[i] = amount / (componentPrice / 10 ** 18) * weights[i] / 100;

            for(uint256 j = 0; j < modules.length; j++) {
                try ITradeModule(modules[j]).trade(
                    setToken,
                    "UNISWAP_TRANSFER_FEE",
                    address(USDC_TOKEN),
                    quantities[i],
                    component,
                    0,
    ...
    function get_test() external view returns (address[] memory) {
        return test;
    }
}